...
int irc_send(int server, char* msg, size_t len) {
	if (send(server, msg, len, 0) < 0) return -1;
	return 0;
}
...
int irc_login(int server, LOGIN_INFO* info) {
	...
	if (irc_send(server, cmd_user, strlen(cmd_user)) < 0) return -1;
	if (irc_send(server, cmd_nick, strlen(cmd_nick)) < 0) return -1;
...
int irc_join(int server, char* channel) {
	...
	if (irc_send(server, cmd_join, strlen(cmd_join)) < 0) return -1;
	return 0;
}
...
int irc_pong(int server, char* msg, size_t len) {
	...
	if (irc_send(server, cmd_pong, strlen(cmd_pong)) < 0) return -1;
	return 0;
}
