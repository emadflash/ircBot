...
void TEST_READ_FROM_FILE_AND_PARSE() {
	...
	while (fgets(buffer, 2000, fp) != NULL) {
		if (is_string_vaild_for_parse(buffer, strlen(buffer)) < 0) continue;
		else {
...
void TEST_VAILD_MESSAGE(char* p) {
	...
	if ( d < 0 ) printf("TEST_VAILD_MESSAGE() not vaild\n");
	else printf("TEST_VAILD_MESSAGE() vaild\n");
	free(b);
}

int main() {
	char* msg = ":drmessano!drmessano@pdpc/supporter/active/drmessano PRIVMSG ##networking :I think they had 8086 CPUs in them, because when you enabled";
